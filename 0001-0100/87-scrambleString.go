...
func main() {
	...
	println(true, isScramble("a", "a"))
	println(isScramble("ccabcbabcbabbbbcbb", "bbbbabccccbbbabcba"))
}
...
func isScrambleRec(s1 string, s2 string, mem map[string]bool) bool {
	if s1 == s2 {
		mem[s1+":"+s2] = true
		return true
	}
	if v, ex := mem[s1+":"+s2]; ex {
		return v
	}
	n := len(s1)

	for i := 1; i < n; i++ {
		if (isScrambleRec(s1[:i], s2[:i], mem) && isScrambleRec(s1[i:], s2[i:], mem)) ||
			(isScrambleRec(s1[:i], s2[n-i:], mem) && isScrambleRec(s1[i:], s2[:n-i], mem)) {
			mem[s1+":"+s2] = true
			return true
		}
	}
...
func isScramble(s1 string, s2 string) bool {
	return isScrambleRec(s1, s2, make(map[string]bool))
}
