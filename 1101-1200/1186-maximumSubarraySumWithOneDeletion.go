...
func maximumSum(arr []int) int {
	cSumWithoutDeletion := arr[0]
	cSumWithDeletion := arr[0]
	maxSumWithDeletion := arr[0]
	maxSumWithoutDeletion := arr[0]

	for i := 1; i < len(arr); i++ {
		cSumWithDeletion = max(cSumWithoutDeletion, cSumWithDeletion+arr[i])
		cSumWithoutDeletion = max(arr[i], cSumWithoutDeletion+arr[i])

		maxSumWithDeletion = max(maxSumWithDeletion, cSumWithDeletion)
		maxSumWithoutDeletion = max(maxSumWithoutDeletion, cSumWithoutDeletion)
	}

	return max(maxSumWithDeletion, maxSumWithoutDeletion)
}
